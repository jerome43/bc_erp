...
export class ListQuotationComponent implements OnInit, OnDestroy {
  ...
  private displayedColumns: string[] = ['id', 'client', 'contact', 'quotationDate', 'edit', 'delete']; // colones affichées par le tableau
  ...
  private dataSource : MatTableDataSource<QuotationId>; // source de données du tableau
  private quotationTypeParams={path : "quotations",isArchived:'false', displayInTemplate:"Devis en cours"}; // les paramètres liés au type de devis (archivés ou courant)
  ...
  initFbQuotations() {
    ...
    this.fbQuotationsSubscription = this.fbQuotations.subscribe((quotations)=>{
      ...
      quotations.forEach((quotation)=>{
        ...
        this.quotationsData.push({id, client, contact, quotationDate});
      });
      this.dataSource = new MatTableDataSource<QuotationId>(this.quotationsData);
