...
export class PdfService {
  ...
  generateDeliveryReceiptPdf(formValue) { // génération du bon de livraison
    ...
    if (formValue.singleProduct.find(e=>e.type === "sale") === undefined && formValue.compositeProducts.find(e=>e.type === "sale") === undefined) { // si aucun élément de type vente est trouvé, on génère un bon de livraion / préparation en vue d'une location
      this.pdfType === PdfType.deliveryReceipt ? tableProducts = [['', {
        alignment: 'center',
        bold: true,
        text: 'Quantité'
    ...
    for (var i = 0; i < formValue.singleProduct.length; i++) {
      if (formValue.singleProduct[i].type !== "service") { // on ne fait pas apparaître les prestations de service dans les bons de prépararion ou de livraions
        ...
        if (!isSaleDeliveryReceipt) {
          this.pdfType === PdfType.deliveryReceipt ? singleProductRow.push(location, '') : singleProductRow.push(location, immobilisation);
        }
    ...
    // PRODUITS COMPOSES
    for (var idxPdt = 0; idxPdt < formValue.compositeProducts.length; idxPdt++) {
      for (var i = 0; i < formValue.compositeProducts[idxPdt].compositeProductElements.length; i++) {
        if (formValue.compositeProducts[idxPdt].compositeProductElements[i].type !== "service") {// on ne fait pas apparaître les prestations de service dans les bons de prépararion ou de livraions
          ...
          if (!isSaleDeliveryReceipt) {
            this.pdfType === PdfType.deliveryReceipt ? compositeProductRow.push(location, '') : compositeProductRow.push(location, immobilisation);
          }
  ...
  generateOrderOrQuotationPdf(formValue) { // génération du devis ou de la facture
    ...
    var options = ""; // for optional products
    var tableOptionalProducts = [['', '', '', '']]; // optional products
    ...
    switch (this.pdfType) {
      ...
      case PdfType.quotation:
        ...
        if (formValue.optionalProduct != undefined && formValue.optionalProduct.length > 0 && formValue.optionalProduct[0] != '') {
          let tableOptionalProducts = [['', {alignment: 'center', bold: true, text: 'Quantité'}, {
            alignment: 'right',
            bold: true,
            text: 'Prix unitaire'
        ...
        for (var i = 0; i < formValue.optionalProduct.length; i++) {
          if (formValue.optionalProduct[i] != '' && formValue.optionalProduct[i] != undefined) {
            var optionalProductRow = [];
            ...
            if (numberOfRentDays > 1) {
              price != undefined ? optionalProductRow.push({
                alignment: 'right',
                text: this.formatToTwoDecimal(price * degressivity) + ' €HT'
              }) : optionalProductRow.push('');
              (price != undefined && formValue.optionalProductAmount[i] != undefined) ? optionalProductRow.push({
                alignment: 'right',
                text: this.formatToTwoDecimal(price * formValue.optionalProductAmount[i] * degressivity) + ' €HT'
              }) : optionalProductRow.push('');
            }
            else {
              price != undefined ? optionalProductRow.push({
                alignment: 'right',
                text: this.formatToTwoDecimal(price * numberOfRentDays) + ' €HT'
              }) : optionalProductRow.push('');
              (price != undefined && formValue.optionalProductAmount[i] != undefined) ? optionalProductRow.push({
                alignment: 'right',
                text: this.formatToTwoDecimal(price * formValue.optionalProductAmount[i] * numberOfRentDays) + ' €HT'
              }) : optionalProductRow.push('');
            }
    ...
    var tableProducts = [['', {alignment: 'center', bold: true, text: 'Quantité'}, {
      alignment: 'right',
      bold: true,
      text: 'Prix unitaire'
    }, {alignment: 'right', bold: true, text: 'Prix HT'}]];
    ...
    for (var idxPdt = 0; idxPdt < formValue.compositeProducts.length; idxPdt++) {
      if (formValue.compositeProducts[idxPdt].compositeProductElements[0] != '') {
        ...
        let price = this.computePriceService.getCompositeProductElementPrice(formValue.compositeProducts[idxPdt].compositeProductElements);
        ...
        if (numberOfRentDays > 1) {
          ...
          (price != undefined && formValue.compositeProductAmount[idxPdt] != undefined) ? compositeProductRow.push({
            alignment: 'right',
            text: this.formatToTwoDecimal(price * formValue.compositeProductAmount[idxPdt] * degressivity) + ' €HT'
          }) : compositeProductRow.push('');
        }
        else {
          price != undefined ? compositeProductRow.push({
          ...
          price != undefined && formValue.compositeProductAmount[idxPdt] != undefined ? compositeProductRow.push({
            alignment: 'right',
            text: this.formatToTwoDecimal(price * formValue.compositeProductAmount[idxPdt] * numberOfRentDays) + ' €HT'
          }) : compositeProductRow.push('');
        }
      ...
      for (var i = 1; i < formValue.compositeProducts[idxPdt].compositeProductElements.length; i++) {
    ...
    formValue.rentalDiscountAmount>0? pricesStack.push({columns: [{width:'65%', text: 'Remise sur location '+formValue.rentalDiscount+'% '}, {width:'35%', alignment: 'right', text: this.formatToTwoDecimal(formValue.rentalDiscountAmount)+' €'}]}) : console.log("pas de remise affichée");
    formValue.saleDiscountAmount>0? pricesStack.push({columns: [{width:'65%', text: 'Remise sur vente '+formValue.saleDiscount+'% '}, {width:'35%', alignment: 'right', text: this.formatToTwoDecimal(formValue.saleDiscountAmount)+' €'}]}) : console.log("pas de remise affichée");
    (formValue.rentalDiscountAmount>0 || formValue.saleDiscountAmount>0) ?  pricesStack.push({columns: [{width:'65%', text: 'Total HT remisé '}, {width:'35%', alignment: 'right', text: this.formatToTwoDecimal(formValue.discountPrice)+' €'}]}): console.log("pas de remise affichée");
    switch (this.pdfType) {
      case PdfType.quotation:
      ...
      case PdfType.advanceInvoice:
        ...
        pricesStack.push({columns: [{width:'65%', text: 'TVA 20% '}, {width:'35%', alignment: 'right', text: this.formatToTwoDecimal(formValue.discountPrice * 0.20 / 100 * formValue.advanceRate) + ' €'}]});
        pricesStack.push({columns: [{width:'65%', bold:true, text : 'TOTAL TTC ', decoration: 'underline'}, {width:'35%', bold:true, alignment:'right', text: this.formatToTwoDecimal(formValue.discountPrice * 1.20 / 100 * formValue.advanceRate) + ' €',  decoration: 'underline'}]});
        break;
      case PdfType.balanceInvoice:
        if (formValue.credit != 0 && typeof formValue.credit === "number") {
          ...
          pricesStack.push({columns: [{width:'65%', bold:true, text:'TOTAL TTC ' , decoration: 'underline'}, {width:'35%', bold:true, alignment:'right',text: this.formatToTwoDecimal((formValue.discountPrice * 1.20 / 100*(100-formValue.advanceRate)) - formValue.credit) + ' €',  decoration: 'underline'}]});
        } else {
          if (formValue.advanceRate>0 && typeof formValue.advanceRate === "number") {pricesStack.push({columns: [{width:'65%', text: 'Solde (' +(100-formValue.advanceRate)+'%) HT '}, {width:'35%', alignment: 'right', text: this.formatToTwoDecimal(formValue.discountPrice / 100*(100-formValue.advanceRate)) + ' €'}]})};
          pricesStack.push({columns: [{width:'65%', text: 'TVA 20% '}, {width:'35%', alignment: 'right', text: this.formatToTwoDecimal(formValue.discountPrice * 0.20 / 100 * (100-formValue.advanceRate)) + ' €'}]});
          pricesStack.push({columns: [{width:'65%', bold:true, text:'TOTAL TTC ' , decoration: 'underline'}, {width:'35%', alignment:'right', text: this.formatToTwoDecimal(formValue.discountPrice * 1.20 / 100*(100-formValue.advanceRate)) + ' €',  decoration: 'underline'}]});
        }
         break;
    }
